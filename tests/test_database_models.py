...
def test_user_interaction_model(session):
    ...
    interaction = UserInteraction(
        ...
        metadata={"duration": 120, "completion": 0.8},
    )
    ...
    assert result.metadata == {"duration": 120, "completion": 0.8}
    assert result.created_at is not None
...
def test_lesson_model(session):
    ...
    lesson = Lesson(
        ...
        metadata={"difficulty": "beginner", "tags": ["test", "lesson"]},
    )
    session.add(lesson)
    session.commit()
    ...
    assert result.metadata == {"difficulty": "beginner", "tags": ["test", "lesson"]}
    assert result.created_at is not None
    assert result.updated_at is not None
    assert result.is_active is True
...
def test_lesson_feedback_model(session):
    ...
    assert result.metadata == {"completion_time": 300}
    assert result.created_at is not None
...
def test_audit_log_model(session):
    ...
    audit_log = AuditLog(
        ...
        status="success",
        details={"document_id": 123, "source": "example.com"},
    )
    ...
    assert result.details == {"document_id": 123, "source": "example.com"}
    assert result.created_at is not None
...
def test_maintenance_log_model(session):
    ...
    maintenance_log = MaintenanceLog(
        ...
        details={"threshold": 0.95, "duration": 120},
    )
    ...
    assert result.details == {"threshold": 0.95, "duration": 120}
    assert result.created_at is not None
